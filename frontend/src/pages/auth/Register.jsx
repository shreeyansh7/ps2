...
export default function Register() {
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'var(--space-lg)',background:'var(--bg-base)',position:'relative'}}>
      <div className="blob-container"><div className="blob blob-1" style={{background:'var(--accent-teal)'}} /><div className="blob blob-2" /></div>
      <div className="animate-fade-up" style={{width:'100%',maxWidth:420,background:'var(--bg-card)',border:'1px solid var(--border-subtle)',borderRadius:'var(--radius-xl)',padding:'var(--space-2xl)',position:'relative',zIndex:1}}>
        <div style={{textAlign:'center',marginBottom:'var(--space-xl)'}}>
          ...
          <h1 style={{fontFamily:'var(--font-display)',fontSize:26,fontWeight:700,letterSpacing:'-0.02em'}}>Create account</h1>
          <p style={{color:'var(--text-secondary)',fontSize:14,marginTop:6}}>Start simulating economic policies</p>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:'var(--space-md)'}}>
          <div className="input-group">
            <label className="input-label">Full Name</label>
            <input className="input" type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" onKeyDown={e=>e.key==='Enter'&&submit()} />
          </div>
          <div className="input-group">
          ...
          <div className="input-group">
            ...
            <input className="input" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 6 characters" onKeyDown={e=>e.key==='Enter'&&submit()} />
          </div>
          ...
          <p style={{textAlign:'center',fontSize:13,color:'var(--text-muted)'}}>Have an account? <Link to="/login" style={{color:'var(--accent-primary)'}}>Sign in</Link></p>
        </div>
      </div>
    </div>
  );
